...
class Model():
    def __init__(self, args, n_items, is_train=True):
        ...
        ## For Masking
        self.start = tf.placeholder(tf.int32, [args.batch_size])
        self.end = tf.placeholder(tf.int32, [args.batch_size])
        self.chk = tf.placeholder(tf.int32, [args.batch_size])
        self.mask = tf.math.equal(self.start, self.end)
        self.mask_num = tf.count_nonzero(self.mask)
        self.chk_mask = tf.math.greater_equal(self.start, self.chk)
        
        ## Initializing Parameters
        sigma = args.sigma if args.sigma != 0 else np.sqrt(6.0 / n_items + args.hidden_size)
